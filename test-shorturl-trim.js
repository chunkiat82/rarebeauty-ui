...
console.log('Password character analysis:');
for (let i = 0; i < password.length; i++) {
  console.log(`Character at position ${i}: '${password[i]}' (ASCII: ${password.charCodeAt(i)})`);
}

// Trim password and show again
const trimmedPassword = password.trim();
console.log('\nTrimmed password character analysis:');
for (let i = 0; i < trimmedPassword.length; i++) {
  console.log(`Character at position ${i}: '${trimmedPassword[i]}' (ASCII: ${trimmedPassword.charCodeAt(i)})`);
}
...
async function testDirectURL() {
  try {
    ...
    // Use the trimmed password
    const directUrl = `https://go.salon.sg/yourls-api.php?username=${encodeURIComponent(username.trim())}&password=${encodeURIComponent(trimmedPassword)}&action=shorturl&format=simple&url=${encodeURIComponent(testURL)}`;
